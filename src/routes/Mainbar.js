...
export default function MainTasks() {
  ...
  const FILTER_MAP = {
    ...
    '/major': (task) => task.completed === false && task.remove === false && task.major === 'true',
    '/completed': (task) => task.completed === true && task.remove === false,
    '/removed': (task) => task.remove === true,
    '/productivity': (task) => task.completed === false && task.remove === false && task.productivity === 'true',
    '/education': (task) => task.completed === false && task.remove === false && task.education === 'true',
    '/health': (task) => task.completed === false && task.remove === false && task.health === 'true',
    '/due': (task) => task.completed === false && task.remove === false && task.due === 'true',
  };
          ...
          Tasks.length > 0
            ? (
              Tasks
                ...
                .slice(0, next)
                .map((Task, index) =>
                  <TaskItem Task={Task} key={Task.id} setTasks={setTasks} index={index} moveTaskListItem={moveTaskListItem} />)
            ) : (<h3>Список задач пуст</h3>)
        }</div>
      {next <= Tasks.length && Tasks.length > 0 ?
        (<div className='main-content--button'><button onClick={() => handleMoreTask()} >показать еще 5 задач</button></div>)
        :
        <div className='main-content--button'><h3>Задач больше нет</h3></div>
      }
    </div>
  )
}